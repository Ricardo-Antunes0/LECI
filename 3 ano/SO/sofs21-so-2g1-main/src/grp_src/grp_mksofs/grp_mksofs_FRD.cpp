...
namespace sofs21
{
    ...
    void grpFillInRootDir(uint32_t ntotal, uint16_t itsize, uint32_t dbtotal)
    {
        ...
        SODirectorySlot directory[DPB];
        memset(directory,0,BlockSize);
        ...
        {
            directory[i].in=NullInodeReference;
            strcpy(directory[i].nameBuffer, "\0");
        }
        soWriteRawBlock(ntotal-dbtotal,directory);
    }
};
